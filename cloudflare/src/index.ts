...
async function verifyJwt(token: string, secret: string): Promise<AuthClaims | null> {
  ...
  try {
    ...
    if (claims.type !== "access" || !claims.sub || !claims.sid || claims.exp <= Math.floor(Date.now() / 1000)) {
      return null;
    }
    return claims;
  } catch {
...
async function verifyPassword(password: string, stored: string) {
  ...
  if (scheme !== "pbkdf2-sha256" || !iterationsText || !salt || !expected) return false;
...
async function getUser(env: Env, userId: string): Promise<UserRow | null> {
  return env.DB.prepare(`
    SELECT id, first_name, last_name, email, city, country, role,
           email_verified_at, totp_secret, totp_enabled, subscription_status, trial_started_at,
...
async function requireAuth(request: Request, env: Env) {
  ...
  `).bind(claims.sid, claims.sub, iso()).first<{ id: string; user_id: string }>();
...
async function verifyTotp(secret: string, code: string) {
  ...
  for (const delta of [-1, 0, 1]) {
    if ((await totpCode(secret, counter + delta)) === normalized) return true;
  }
  return false;
}

function totpUri(secret: string, email: string) {
  return `otpauth://totp/Theo:${encodeURIComponent(email)}?secret=${secret}&issuer=Theo&algorithm=SHA1&digits=6&period=30`;
}

async function issueSession(env: Env, user: UserRow, deviceId: string | null, deviceLabel: string | null) {
  ...
  await env.DB.prepare(`
    INSERT INTO sessions
      (id,user_id,refresh_token_hash,device_id,device_label,expires_at,last_used_at,created_at)
    VALUES (?,?,?,?,?,?,?,?)
  `).bind(
    sessionId, user.id, refreshHash, deviceId, deviceLabel,
    iso(new Date(nowDate.getTime() + refreshTtl * 1000)), iso(nowDate), iso(nowDate),
  ).run();

  if (deviceId) {
    await env.DB.prepare(`
      INSERT INTO devices(id,user_id,label,last_seen_at,created_at)
      VALUES(?,?,?,?,?)
      ON CONFLICT(user_id,id) DO UPDATE SET label=excluded.label,last_seen_at=excluded.last_seen_at
    `).bind(deviceId, user.id, deviceLabel, iso(), iso()).run();
  }
...
async function authRegister(request: Request, env: Env) {
  ...
  if (!email || password.length < 8 || !firstName || !lastName || !cpf || !city || !country) {
    return jsonResponse({ message: "Dados de cadastro inválidos." }, 422, env);
  }

  const existing = await env.DB.prepare("SELECT id FROM users WHERE email = ? OR cpf = ?")
    .bind(email, cpf).first();
  if (existing) return jsonResponse({ message: "E-mail ou CPF já cadastrado." }, 409, env);
  ...
  await env.DB.prepare(`
    INSERT INTO users
      (id,first_name,last_name,email,cpf,country,city,password_hash,
       trial_started_at,trial_expires_at,subscription_status,created_at,updated_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).bind(
    id, firstName, lastName, email, cpf, country, city, passwordHash,
    iso(started), iso(expires), "trial", iso(started), iso(started),
  ).run();

  const user = await getUser(env, id);
  return jsonResponse({
    message: "Conta criada. Seu período gratuito de 7 dias começou.",
...
async function authLogin(request: Request, env: Env) {
  ...
  // Não diferencia usuário inexistente de senha incorreta.
  if (!user || !(await verifyPassword(password, user.password_hash))) {
    return jsonResponse({ message: "E-mail ou senha inválidos." }, 401, env);
  }

  const publicRow = await getUser(env, user.id);
  if (!publicRow) return jsonResponse({ message: "Conta não encontrada." }, 404, env);
  ...
  if (Boolean(user.totp_enabled) && user.totp_secret) {
    ...
    await env.DB.prepare(`
      INSERT INTO twofa_challenges(id,user_id,token_hash,expires_at,created_at)
      VALUES(?,?,?,?,?)
    `).bind(
      uuid(),
      publicRow.id,
      challengeHash,
      iso(new Date(Date.now() + 5 * 60_000)),
      iso(),
    ).run();
...
async function authRefresh(request: Request, env: Env) {
  ...
  if (!token) return jsonResponse({ message: "Refresh token ausente." }, 401, env);
  ...
  `).bind(hash, iso()).first<any>();

  if (!session) return jsonResponse({ message: "Sessão expirada." }, 401, env);

  const user = await getUser(env, session.uid);
  if (!user) return jsonResponse({ message: "Conta não encontrada." }, 404, env);

  const newRefresh = await randomToken();
  const newHash = await sha256Hex(newRefresh);
  await env.DB.prepare(`
    UPDATE sessions SET refresh_token_hash = ?, last_used_at = ? WHERE id = ?
  `).bind(newHash, iso(), session.id).run();

  const issued = Math.floor(Date.now() / 1000);
  const accessToken = await signJwt({
    sub: user.id, sid: session.id, type: "access", iat: issued,
...
async function authMe(request: Request, env: Env) {
  ...
  if (!user) return jsonResponse({ message: "Não autenticado." }, 401, env);
...
async function auth2faEnable(request: Request, env: Env) {
  ...
  if (!user.totp_secret || !(await verifyTotp(user.totp_secret, code))) {
    return jsonResponse({ message: "Código inválido." }, 422, env);
  }

  await env.DB.prepare("UPDATE users SET totp_enabled = 1, updated_at = ? WHERE id = ?")
    .bind(iso(), user.id).run();

  return jsonResponse({ message: "2FA ativado." }, 200, env);
}
...
async function auth2faDisable(request: Request, env: Env) {
  ...
  return jsonResponse({ message: "2FA desativado." }, 200, env);
}
...
async function auth2faVerify(request: Request, env: Env) {
  ...
  if (!challengeToken || !code) {
    return jsonResponse({ message: "Desafio ou código ausente." }, 422, env);
  }

  const hash = await sha256Hex(challengeToken);
  const challenge = await env.DB.prepare(`
    SELECT id,user_id FROM twofa_challenges
    WHERE token_hash = ? AND consumed_at IS NULL AND expires_at > ?
  `).bind(hash, iso()).first<{ id: string; user_id: string }>();

  if (!challenge) return jsonResponse({ message: "Desafio inválido ou expirado." }, 401, env);
  ...
  if (!user?.totp_enabled || !user.totp_secret || !(await verifyTotp(user.totp_secret, code))) {
    return jsonResponse({ message: "Código inválido." }, 401, env);
  }

  await env.DB.prepare("UPDATE twofa_challenges SET consumed_at = ? WHERE id = ?")
    .bind(iso(), challenge.id).run();
...
async function authLogout(request: Request, env: Env) {
  ...
  const header = request.headers.get("authorization")!;
  const claims = await verifyJwt(header.slice(7), env.JWT_SECRET);
  if (claims) await env.DB.prepare("UPDATE sessions SET revoked_at = ? WHERE id = ?").bind(iso(), claims.sid).run();

  return jsonResponse({ message: "Sessão encerrada." }, 200, env);
}

async function subscriptionStatus(request: Request, env: Env) {
  const user = await requireAuth(request, env);
  if (!user) return jsonResponse({ message: "Não autenticado." }, 401, env);
  return jsonResponse({ ...publicUser(env, user), access: accessFor(env, user) }, 200, env);
}
...
async function syncPush(request: Request, env: Env) {
  ...
  if (events.length === 0) return jsonResponse({ accepted: 0, ignored: 0 }, 200, env);
  if (events.length > 100) return jsonResponse({ message: "Máximo de 100 eventos por lote." }, 413, env);

  const deviceId = String(body?.device_id ?? "").trim();
  if (!deviceId) return jsonResponse({ message: "device_id é obrigatório." }, 422, env);
  ...
  const stmt = env.DB.prepare(`
    INSERT OR IGNORE INTO sync_events
      (event_id,user_id,device_id,entity_type,entity_id,operation,version,payload_json,created_at)
    VALUES (?,?,?,?,?,?,?,?,?)
  `);

  const prepared: { eventId: string; statement: D1PreparedStatement }[] = [];
  ...
  for (const event of events) {
    ...
    if (!eventId || eventId.length > 100 || !allowedEntities.has(entityType) ||
        entityId.length > 200 || !allowedOps.has(operation) ||
        !Number.isSafeInteger(version) || version < 0) {
      return jsonResponse({ message: "Evento inválido." }, 422, env);
    }
    ...
    prepared.push({
      eventId,
      statement: stmt.bind(
        eventId, user.id, deviceId, entityType, entityId, operation, version, payload, created,
      ),
    });
  }
  ...
  results.slice(0, prepared.length).forEach((result, index) => {
    if (result.meta.changes === 1) accepted.push(prepared[index].eventId);
    else ignored.push(prepared[index].eventId);
  });

  return jsonResponse({ accepted: accepted.length, ignored: ignored.length, event_ids: accepted }, 200, env);
}
...
async function syncPull(request: Request, env: Env) {
  ...
  const rows = await env.DB.prepare(`
    SELECT seq,event_id,device_id,entity_type,entity_id,operation,version,payload_json,created_at
    FROM sync_events
  ...
  return jsonResponse({
    ...
    next_cursor: rows.results.length ? Number((rows.results[rows.results.length - 1] as any).seq) : since,
...
async function syncBootstrap(request: Request, env: Env) {
  ...
  const rows = await env.DB.prepare(`
    SELECT seq,event_id,device_id,entity_type,entity_id,operation,version,payload_json,created_at
    FROM sync_events WHERE user_id = ? AND seq > ? ORDER BY seq ASC LIMIT ?
  `).bind(user.id, since, limit).all();

  const lastSeq = await env.DB.prepare(
    "SELECT MAX(seq) AS max_seq FROM sync_events WHERE user_id = ?",
  ).bind(user.id).first<{ max_seq: number | null }>();
  ...
  return jsonResponse({
    ...
    next_cursor: rows.results.length ? Number((rows.results[rows.results.length - 1] as any).seq) : 0,
...
async function paymentWebhook(request: Request, env: Env) {
  ...
  // A integração futura deve validar assinatura/autenticidade do provedor
  // antes de alterar subscription_status.
  const secret = request.headers.get("x-theo-webhook-secret");
  if (!env.WEBHOOK_SECRET || secret !== env.WEBHOOK_SECRET) {
    return jsonResponse({ message: "Webhook não autorizado." }, 401, env);
  }
  ...
  if (!userId || !["active","expired","cancelled","past_due"].includes(status)) {
    return jsonResponse({ message: "Evento de assinatura inválido." }, 422, env);
  }
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (request.method === "POST" && path === "/auth/refresh") return authRefresh(request, env);
      if (request.method === "POST" && path === "/auth/logout") return authLogout(request, env);
      if (request.method === "POST" && path === "/auth/2fa/setup") return auth2faSetup(request, env);
      if (request.method === "POST" && path === "/auth/2fa/enable") return auth2faEnable(request, env);
      if (request.method === "POST" && path === "/auth/2fa/disable") return auth2faDisable(request, env);
      if (request.method === "POST" && path === "/auth/2fa/verify") return auth2faVerify(request, env);
      if (request.method === "GET" && (path === "/auth/me" || path === "/users/me")) return authMe(request, env);
      if (request.method === "GET" && path === "/subscription/status") return subscriptionStatus(request, env);
      ...
      if (request.method === "GET" && path === "/sync/bootstrap") return syncBootstrap(request, env);

      if (request.method === "POST" && path === "/webhooks/subscription") return paymentWebhook(request, env);

      return jsonResponse({ message: "Rota não encontrada." }, 404, env);
    } catch (error) {
      console.error("[Theo Worker]", error);
      return jsonResponse({ message: "Erro interno." }, 500, env);
    }
  },
};
