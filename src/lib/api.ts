...
import type { AuthResponse, Requires2FAResponse, RegisterResponse, UserPublic } from "./types";
...
// Em caso de 401, tenta renovar o token uma única vez e refaz a requisição original.
...
export function extractErrorMessage(err: unknown): string {
  if (axios.isAxiosError(err)) {
    const body = err.response?.data as
      | { message?: string; error?: string | { code?: string; message?: string }; detail?: string }
      | undefined;
    ...
    if (err.response?.status === 404) return "O deck ou a pasta selecionada não foi encontrado.";
...
// ============================================================
// DADOS LOCAIS (offline) — decks, cards, estudo, editais, etc.
// ============================================================
...
// ============================================================
// IMPORTAÇÃO/EXPORTAÇÃO DE PLANILHA — exige servidor
// ============================================================
...
// ============================================================
// AUTENTICAÇÃO / CONTA — exige servidor
// ============================================================
...
export async function logout(): Promise<void> {
  ...
  if (tokens?.refresh_token) {
    try {
      await http.post("/auth/logout", { refresh_token: tokens.refresh_token });
    } catch {
...
// ============================================================
// COMUNIDADE — exige servidor + conta
// ============================================================
...
export async function searchCommunityListings(query?: string): Promise<CommunityListing[]> {
  const { data } = await http.get<{ listings: CommunityListing[] }>("/community/listings", {
    params: { q: query },
  });
...
// ============================================================
// ADMIN — exige servidor + conta admin
// ============================================================
...
export async function adminSetDeckTemplate(deckId: string, isTemplate: boolean) {
  const { data } = await http.patch(`/admin/decks/${deckId}/template`, { is_template: isTemplate });
  return data;
}
...
// ============================================================
// IA (Theo) e ASSINATURA — exigem servidor
// ============================================================
